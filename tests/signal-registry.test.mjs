...
describe("EXACT_REGISTRY — per-entry completeness", () => {
  it("holds the seed (14) + Wave-B NS email (3) + v0.6.0 keyspace (250) + PHASE-B1 violation types (29) + PHASE-B1B corrections (2) + HS-22-VERBS (22) + PR-B-α canonical (3) + PQP doc-mirror (1) = 327 canonical types (+3 OHH attendee tuples: attendee_signed_in/attendee_assessment_completed/lead_interested)", () => {
    ...
    // so 370→371.
    // v0.25.0 (SIGNALS-REGISTER-PFP-VA-LEAD-SAVED, +1): LIVE-emitted by PFP (the VA
    // deepen-trio save already shipped + firing), registered to fix classification
    // from the DEFAULT weight-3 fallback — `pathfinder-pro.va_lead_saved` (w3,
    // BEHAVIORAL, goalShift:true, active; top-of-funnel VA / veteran lead-SAVE
    // action, the direct sibling of pathfinder-pro.dscr_lead_saved +
...
describe("listActiveSignalTypes", () => {
  ...
  it("active count = 320 (328 total − 8 forensic)", () => {
    ...
    // v0.25.0 (SIGNALS-REGISTER-PFP-VA-LEAD-SAVED, +1): LIVE-emitted by PFP —
    // pathfinder-pro.va_lead_saved (w3 BEHAVIORAL, top-of-funnel VA / veteran
    // lead-SAVE action, the direct sibling of pathfinder-pro.dscr_lead_saved +
    // pathfinder-pro.hecm_lead_saved + pathfinder-pro.bankstatement_lead_saved,
    // classified identically to pathfinder-pro.quick_estimate_completed)
